<x-app-layout>
                    ...
                    <div class="cart">
                        @isset ($items)
                                    ...
                                    @for ($x = 0; $x < count($items); $x++)
                                        <tr item-id="{{ $items[$x]['id'] }}">
                                            <td class="img-button" data-label="Item">
                                                ...
                                                <x-button class="cart-btn" item-id="{{ $items[$x]['id'] }}" type="button">{{ __('messages.Remove From Cart') }}</x-button>
                                            </td>
                            ...
                            <div class="cart-order">
                                <h1 class="cart-total"></h1>
                                <a href="{{ url('order/create') }}"><x-button type="button">{{ __('messages.Make Order') }}</x-button></a>
                            </div>
                        @endisset
                    </div>     
                </div>
    ...
    <script>
        ...
        function updateTotal () {
            ...
            $('.cart-total').text("{{ __('messages.Total') }}: " + total + '.00 €');
        }

        function checkItemCount() {
            if (!$('tr[item-id]').length) {
                $('.table').remove();
                $('.cart').text('{{ __("messages.No items added to the cart!") }}');
            }
        }
